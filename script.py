...
class Scraper:
    ...
    def get_num_pages(self):
        ...
        try:
            ...
            url = 'https://www.examtopics.com/discussions/amazon/'  # Replace with your target URL
            response = scraper.get(f"{self.base_url}")
            #print(response)
            #response = self.session.get(f"{self.base_url}")
            #print(response)
            #response = self.session.get("https://www.examtopics.com/discussions/amazon/")
    ...
    def fetch_page_links(self, page, search_string):
        ...
        try:
            scraper = cs.create_scraper()
            response = scraper.get(f"{self.base_url}{page}/")
            soup = BeautifulSoup(response.content, "html.parser")
